...
# TODO: извлекать плотности
class Mask2ImageDataset(torch.utils.data.Dataset):
    def __init__(self, images_dir: str, masks_dir: str, width=224, height=224):
        self.width = width
        self.height = height
...
if __name__ == "__main__":
    config = OmegaConf.load(r"F:\ITMO_ML\CTCI-generation\configs\mask_sdipa_config.yaml")
    ...
    vae = load_vqvae_from_checkpoint(vqvae_checkpoint)
    ...
    train_dataset = Mask2ImageDataset(train_images_dir, train_masks_dir)
    val_dataset =Mask2ImageDataset(val_images_dir, val_masks_dir)
